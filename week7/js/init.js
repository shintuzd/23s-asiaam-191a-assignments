...
L.control.layers(null,layers).addTo(map3)
...
function addMarker(data, target_map=1){
    if (target_map == 3){
        if(data["What's your name?"] == "Anonymous"){
            circleOptions.fillColor = "red"
            anon.addLayer(L.circleMarker([data.lat,data.lng],circleOptions).bindPopup(`<h2>${data["What's the name of your favorite restaurant?"]}</h2> <h3>Submitted by: ${data["What's your name?"]}</h3> <p>What I Like About It: ${data["What do you like about it?"]}</p>`))
            createButtons2(data.lat,data.lng,data["What's the name of your favorite restaurant?"])
        }
        else{
            circleOptions.fillColor = "blue"
            person.addLayer(L.circleMarker([data.lat,data.lng], circleOptions).bindPopup(`<h2>${data["What's the name of your favorite restaurant?"]}</h2> <h3>Submitted by: ${data["What's your name?"]}</h3> <p>What I Like About It: ${data["What do you like about it?"]}</p>`))
            createButtons2(data.lat,data.lng,data["What's the name of your favorite restaurant?"])
...
function processData(results, tgt_map){
    ...
    anon.addTo(map3) // add our layers after markers have been made
    person.addTo(map3) // add our layers after markers have been made  
    let allLayers = L.featureGroup([anon,person]);
    map3.fitBounds(allLayers.getBounds());
}
